...
from doctruck_backend.api.schemas import UserSchema, FoodTruckSchema, LocationSchema, DocumentSchema
...
api.add_resource(FoodTruckResource, "/food-trucks/<int:truck_id>", endpoint="food_truck_by_id")

# Location 라우트 (공개 API - 인증 불필요)
api.add_resource(LocationList, "/locations", endpoint="locations")
api.add_resource(LocationResource, "/locations/<int:location_id>", endpoint="location_by_id")
...
api.add_resource(AdminDocumentPending, "/admin/documents/pending", endpoint="admin_documents_pending")
api.add_resource(AdminDocumentVerify, "/admin/documents/<int:doc_id>/verify", endpoint="admin_document_verify")
api.add_resource(AdminDocumentList, "/admin/documents", endpoint="admin_documents")
api.add_resource(AdminDocumentResource, "/admin/documents/<int:doc_id>", endpoint="admin_document_by_id")

# Admin Location 라우트 (관리자 전용)
api.add_resource(AdminLocationList, "/admin/locations", endpoint="admin_locations")
api.add_resource(AdminLocationResource, "/admin/locations/<int:location_id>", endpoint="admin_location_by_id")

# Admin User 라우트 (관리자 전용)
api.add_resource(AdminUserList, "/admin/users", endpoint="admin_users")
api.add_resource(AdminUserResource, "/admin/users/<int:user_id>", endpoint="admin_user_by_id")

# Admin Document-Location 연결 (관리자 전용)
api.add_resource(AdminDocumentLocationConnect, "/admin/documents/<int:doc_id>/locations/<int:location_id>", endpoint="admin_doc_location_connect")
api.add_resource(AdminDocumentLocationList, "/admin/documents/<int:doc_id>/locations", endpoint="admin_doc_locations")

# Location Interest (사용자 기능 - 위치 관심 등록)
api.add_resource(LocationInterest, "/locations/<int:location_id>/interest", endpoint="location_interest")
api.add_resource(MyLocationInterests, "/my/interests", endpoint="my_interests")

# Recommendations (사용자 기능 - 맞춤 추천)
api.add_resource(RecommendedLocations, "/recommendations/locations", endpoint="recommended_locations")
api.add_resource(RecommendedDocuments, "/recommendations/documents", endpoint="recommended_documents")
